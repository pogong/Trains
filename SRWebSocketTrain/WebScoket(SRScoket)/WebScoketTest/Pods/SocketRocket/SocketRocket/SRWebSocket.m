...
@interface SRIOConsumer : NSObject {
    stream_scanner _edge_consumer;
    data_callback _handler;
    size_t _bytesNeeded;
    BOOL _readToCurrentFrame;
    BOOL _unmaskBytes;
}
@property (nonatomic, copy, readonly) stream_scanner edge_consumer;
...
#pragma clang diagnostic pop
    ...
    [self _readHTTPHeader];//zc read:不是真正做读取的工作,只是做好读取的准备(就像CocoaAsyncSocket建读取的包一样)
}
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    //zc read:frame_header.payload_length == 0==>pong就是
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(SRWebSocket *self, NSData *data) {
        ...
        //zc read:分水岭 (有Extended payload length)^(没有Extended payload length)
        if (extra_bytes_needed == 0) {
            [self _handleFrameHeader:header curData:self->_currentFrameData];
        } else {
            //zc focus:addConsumer4
            NSLog(@"读Extended data前的header");
            //zc read:读Extended data
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        NSLog(@"zc read:_outputStream write");
...
//                [self _pumpScanner];//zc read:没数据可读,调用_pumpScanner,中途就会return
            ...
            case NSStreamEventErrorOccurred: {
                NSLog(@"zc read:NSStreamEventErrorOccurred %@",aStream);
            ...
            case NSStreamEventEndEncountered: {
                [self _pumpScanner];
                NSLog(@"zc read:NSStreamEventEndEncountered %@",aStream);
            ...
            case NSStreamEventHasSpaceAvailable: {
                NSLog(@"zc read:NSStreamEventHasSpaceAvailable %@",aStream);
...
@synthesize edge_consumer = _edge_consumer;
...
{
    _edge_consumer = [scanner copy];
    _handler = [handler copy];
    _bytesNeeded = bytesNeeded;
    _readToCurrentFrame = readToCurrentFrame;
    _unmaskBytes = unmaskBytes;
    assert(_edge_consumer || _bytesNeeded);
}
...
static NSInteger zc_tag = 0;

- (SRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
    SRIOConsumer *consumer = nil;
    //zc read:有可能不新建,而是拿老的用
