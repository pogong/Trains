...
#pragma mark readSource的回调
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
        ...
        //开始写
        NSLog(@"zc func call writeSource_handler call doWriteData");
...
- (void)suspendWriteSource
{
	if (!(flags & kWriteSourceSuspended))
	{
		...
		NSLog(@"zc func call 暂停对writeSource的监听");
...
- (void)resumeWriteSource
{
	if (flags & kWriteSourceSuspended)
	{
		...
		NSLog(@"zc func call 开启对writeSource的监听");
...
{
    NSLog(@"zc func call 开始写包 %zd",tag);
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		...
		{
            NSLog(@"zc func call 写队列内任务个数 %zd",[writeQueue count]);
			// Dequeue the next object in the write queue
			...
			else
			{
                ...
                NSLog(@"zc func call maybeDequeueWrite call doWriteData");
				[self doWriteData];
			}
		}
		else if (flags & kDisconnectAfterWrites)
...
- (void)zc_doWriteData
{
    ...
    else
    {
        ...
        NSLog(@"zc func call 包数据过大,未完待续");
        
        if (!waiting && !error)
        {
            // This would be the case if our write was able to accept some data, but not all of it.
            
            //zc read:没完(包的数据量比较大,一次没有写出完毕),不等,没错==>socket没有足够的空间来接收包的所有数据,开启对writeSource的监听,一旦有空间就立刻写入
            flags &= ~kSocketCanAcceptBytes;
            [self resumeWriteSource];
        }
