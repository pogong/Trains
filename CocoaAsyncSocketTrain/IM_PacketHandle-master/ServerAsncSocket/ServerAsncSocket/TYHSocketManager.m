...
- (void)socket:(GCDAsyncSocket *)sock didAcceptNewSocket:(GCDAsyncSocket *)newSocket
{
    ...
    NSLog(@"zc func call -- 新建定边界packet读取数据-----------packet start");
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if (!currentPacketHead) {
        ...
        NSLog(@"读头成功,包长:%zd",packetLength);
        //读到数据包的大小
        NSLog(@"zc func call -- packet读取数据结束-----------packet end\n");
        
        NSLog(@"zc func call -- 新建定长packet读取数据-----------packet start");
