               ...
               tag:(long)i
{
	if((self = [super init]))
	{
        ...
        if (d)//zc read:外围提供缓存块
		{
			buffer = d;
			startOffset = s;//zc read:针对自己控制buff的偏移
			bufferOwner = NO;
		...
		{
			if (readLength > 0)
				buffer = [[NSMutableData alloc] initWithLength:readLength];//zc read:外围不提供缓存块,读定长,缓存长度为定长
			else
				buffer = [[NSMutableData alloc] initWithLength:0];//zc read:外围不提供缓存块,读到边界,缓存长度为0
...
- (void)setupReadAndWriteSourcesForNewlyConnectedSocket:(int)socketFD
{
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
        ...
        NSLog(@"zc func call-->source_handler trigger datalength = %zd",strongSelf->socketFDBytesAvailable);
...
- (void)suspendReadSource
{
	if (!(flags & kReadSourceSuspended))
	{
        ...
        NSLog(@"zc func call-->暂停对source的监听");
...
- (void)resumeReadSource
{
	...
	{
		LogVerbose(@"dispatch_resume(readSource)");
		NSLog(@"zc func call-->恢复对source的监听");
		dispatch_resume(readSource);
		flags &= ~kReadSourceSuspended;
	}
}
...
- (void)doReadData
{
	...
	if ((currentRead == nil) || (flags & kReadsPaused))
	{
        ...
        
		{
			...
			if (socketFDBytesAvailable > 0)
			{
                //zc read:当readSource监听到有新的数据来且currentRead为空,会暂停对readSource的监听(不监听不代表数据就不来了)
				[self suspendReadSource];
			}
		}
	...
	if ([preBuffer availableBytes] > 0)
	{
        ...
        NSLog(@"zc func call-->STEP 1 - READ FROM PREBUFFER");
	...
	if (!done && !error && !socketEOF && hasBytesAvailable)
	{
		...
		
		{
            ...
            NSLog(@"zc func call-->STEP 2 - READ FROM SOCKET");
	...
	if (done)
	{
		...
		{
            NSLog(@"zc func call--> doReadData call maybeDequeueRead ^ availableBytes=%zd",[preBuffer availableBytes]);
			[self maybeDequeueRead];
		}
	}
	else if (totalBytesReadForCurrentRead > 0)
