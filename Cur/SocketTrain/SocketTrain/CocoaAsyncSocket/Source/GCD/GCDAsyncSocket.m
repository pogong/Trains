...
- (NSUInteger)readLengthForTermWithHint:(NSUInteger)bytesAvailable shouldPreBuffer:(BOOL *)shouldPreBufferPtr
{
    ...
    //zc read:对比 socket->currentRead->buffer(NSMutableData *buffer) VS socket->preBuffer.
	...
	// 5. Removing underflow from prebuffer (memmove)//清除
	// 
...
@implementation GCDAsyncSocket
{
	...
	dispatch_source_t readSource;//√ dispatch_sourse的作用只是为了监听,真正的读与写还得用socket的C语言的函数
	dispatch_source_t writeSource;//√
    
	dispatch_source_t readTimer;//√ 掐算读取超时
	...
	unsigned long socketFDBytesAvailable;//√收到数据长度
    ...
    //原地址连接失败,延迟多长时间连接备选地址(If a DNS lookup returns both IPv4 and IPv6 results, the preferred protocol will be chosen.备选地址会出现在返回两个IP地址的情况下)
...
- (id)initWithDelegate:(id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		#if !OS_OBJECT_USE_OBJC  //zc read:这个宏是在sdk6.0之后才有的.6.0之前,GCD的对象需要retain+release,6.0之后ARC管理了GCD
...
{
    //zc read:create readSource dispatch_source_create 参数(监听事件类型,socket描述符,监听事件具体操作类型,回调队列)
	readSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, socketFD, 0, socketQueue);
    
    //zc read:create writeSource
    ...
    //zc read:当socket中收到数据流，就会触发这个句柄(dispatch_sourse的作用只是为了监听,真正的读与写还得用socket的C语言的函数)
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
		...
		if (strongSelf->socketFDBytesAvailable > 0)
            [strongSelf doReadData];//zc read:读到内容
		else
			[strongSelf doReadEOF];//zc read:读到结束符
	...
	socketFDBytesAvailable = 0;
    flags &= ~kReadSourceSuspended;//zc read:与'kReadSourceSuspended取非的结果',清除32位内8位前的所有状态标记
...
**/
/*
 1.创建包读取==>[self doReadData]
 2.开启加密时的作用==>[self maybeStartTLS]
 3.加密读取的收尾工作==>[self flushSSLBuffers]+[self resumeReadSource]
 */
...
//zc read:result==0>>>调用doReadEOF
//单个包的结束不是以result==0为结束的,而是[有定长读到了定长],[有边界读到了边界]做为结束的
//result==0>>>表示`对面`想关闭Socket的交互(如:HTTP或者HTTPS在传输完webpage后,都会出现result==0)
- (void)doReadEOF
{
	LogTrace();
...
- (void)completeCurrentRead
{
    ...
    if (currentRead->bufferOwner)//zc read:内部构建的缓存
	{
...
- (void)setupReadTimerWithTimeout:(NSTimeInterval)timeout
{
	if (timeout >= 0.0)
	{
        ...
        dispatch_time_t tt = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeout * NSEC_PER_SEC));//zc read:timeout外围设置
		
        dispatch_source_set_timer(readTimer, tt, DISPATCH_TIME_FOREVER, 0);//1->源,2->开始时间,3->间隔时间,4->误差秒数
...
- (OSStatus)sslReadWithBuffer:(void *)out_pass_buffer length:(size_t *)bufferLength
{
	...
	if (sslPreBufferLength > 0)
	{
		...
		memcpy(out_pass_buffer, [sslPreBuffer readBuffer], bytesToCopy);
	...
	if (!done && (socketFDBytesAvailable > 0))
	{
		...
		else
		{
			...
			if (readIntoPreBuffer)
			{
				...
				memcpy((uint8_t *)out_pass_buffer + totalBytesRead, [sslPreBuffer readBuffer], bytesToCopy);
				[sslPreBuffer didRead:bytesToCopy];
...
//zc read:sslRead2==>SSLReadFunction不自动触发,我们调用了SSLRead,才会调到这.(SSLWriteFunction也一样)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark Security via CFStream //zc read:SSL2
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
+ (void)cfstreamThread {
    @autoreleasepool{
        ...
        //zc read:NSTimer一直存在,RunLoop也一直存在
