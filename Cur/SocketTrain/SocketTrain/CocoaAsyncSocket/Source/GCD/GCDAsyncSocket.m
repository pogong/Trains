...
//zc_flushSSLBuffers可能只是在没有读取包的时候,将加密的数据加入的解密出来,并放入preBuffer
- (void)zc_flushSSLBuffers
{
    LogTrace();
    ...
    dispatch_block_t updateEstimatedBytesAvailable = ^{
        ...
        //zc read:三个数据长度加在一块(原生socket内的+sslPreBuffer内的+sslContext内的)
    ...
    if (estimatedBytesAvailable > 0)
    {
        ...
        do
        {
            ...
            */
            ...
            //zc read:SSLRead没有读到数据,且预估estimatedBytesAvailable为0==>退出循环
...
- (void)zc_doReadData_ST
{
    ...
    
    {
        ...
        if (flags & kSocketSecure)
        {
            //zc read:2 buffers==>sslPreBuffer+[sslContext internal buffer]
